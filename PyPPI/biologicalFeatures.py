...
PRO_DIR='/home/wangyansong/wangyubo/PyPPI'


def generate_ECO(ECO,windowSize):
    cmd="echo ECO computing!"
    os.system(cmd)
    cmd = "chmod +x run_ECO.sh"
    os.system(cmd)
    cmd = PRO_DIR + "/feature_computation/ECO/run_ECO.sh"
    os.system(cmd)
    return enter(ECO,windowSize) # feature_3d_npArray


def generate_RAA(RAA,windowSize):
    cmd="echo RAA computing!"
    os.system(cmd)
    cmd = "chmod +x run_RAA.sh"
    os.system(cmd)
    cmd = PRO_DIR + "/feature_computation/RAA/run_RAA.sh"
    os.system(cmd)
    return enter(RAA,windowSize) # feature_3d_npArray
