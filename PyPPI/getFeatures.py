...
def GetProgramArguments(ECO,RAA,HSP,Pro2Vec_1D,PSSM,WINDOW_SIZE,RSA,Anchor,HYD,PKA,PHY_Char,PHY_Prop,POSITION):
    logging.info("Parsing program arguments...")
    parser = argparse.ArgumentParser(description='getFeatures.py')
    parser._action_groups.pop()

    # required arguments
    required = parser.add_argument_group('required arguments')
    # Define the fasta file path, please replace the default value with your own path
    required.add_argument('-i', '--input_fn', type=str, required=False, default=os.environ.get('INPUT_FN'),
                          help='str: input protein sequences. In fasta format')
    # Define a feature output path, please replace the default value with your own path
    required.add_argument('-o', '--out_dir', type=str, required=False, default=os.environ.get('PRO_DIR')+'/feature_computation/Features',
                          help='str: output directory')
    # Define a temporary feature output path, the corresponding content will be deleted after execution, replace the default value with your own path
    required.add_argument('-d', '--tmp_dir', type=str, required=False, default=os.environ.get('TMP_DIR'),
                          help='str: temporary  directory to store all features. Will be deleted after computing')
    required.add_argument('-w', '--window_size', type=int, required=False, default=WINDOW_SIZE,
                          help='str: different window sizes in features')
    # Pass in the features that need to be generated
    required.add_argument('-Anchor', '--Anchor', type=bool, required=False, default=Anchor,
                          help='str: pass in the features')
    required.add_argument('-ECO', '--ECO', type=bool, required=False, default=ECO,
                          help='str: pass in the features')
    required.add_argument('-HSP', '--HSP', type=bool, required=False, default=HSP,
                          help='str: pass in the features')
    required.add_argument('-HYD', '--HYD', type=bool, required=False, default=HYD,
                          help='str: pass in the features')
    required.add_argument('-PHY_Char', '--PHY_Char', type=bool, required=False, default=PHY_Char,
                          help='str: pass in the features')
    required.add_argument('-PHY_Prop', '--PHY_Prop', type=bool, required=False, default=PHY_Prop,
                          help='str: pass in the features')
    required.add_argument('-PKA', '--PKA', type=bool, required=False, default=PKA,
                          help='str: pass in the features')
    required.add_argument('-POSITION', '--POSITION', type=bool, required=False, default=POSITION,
                          help='str: pass in the features')
    required.add_argument('-Pro2Vec_1D', '--Pro2Vec_1D', type=bool, required=False, default=Pro2Vec_1D,
                          help='str: pass in the features')
    required.add_argument('-PSSM', '--PSSM', type=bool, required=False, default=PSSM,
                          help='str: pass in the features')
    required.add_argument('-RAA', '--RAA', type=bool, required=False, default=RAA,
                          help='str: pass in the features')
    required.add_argument('-RSA', '--RSA', type=bool, required=False, default=RSA,
                          help='str: pass in the features')
...
def Read1DFeature(input_fn, dict):
    ...
    while True:
        ...
        if not line_feature:
            break
        if line_PID[1:] not in ['P84801', 'O67139', 'P23007']: # UniProt IDs for three different proteins
...
def LoadLabelsAndFormatFeatures(args):
    ...
    while True:
        ...
        if not line_Pseq:
            break
        if line_PID[1:] not in ['P84801', 'O67139', 'P23007']:
            if (args.ECO == True):
    ...
    if (args.RAA == True):
        return RAA_3D_np
    if (args.RSA == True):return RSA_3D_np
    if (args.Anchor == True):return Anchor_3D_np
    if (args.HYD == True):return HYD_3D_np
    if (args.PKA == True):return PKA_3D_np
    if (args.Pro2Vec_1D == True):return Pro2Vec_1D_3D_np
    if (args.HSP is True):return HSP_3D_np
    if (args.POSITION == True):return POSITION_3D_np
    if (args.PHY_Char == True):return np.concatenate((PHY_Char_3D_np_1, PHY_Char_3D_np_2, PHY_Char_3D_np_3),axis=2)
    if (args.PHY_Prop == True):return np.concatenate((PHY_Prop_3D_np_1, PHY_Prop_3D_np_2, PHY_Prop_3D_np_3,
            PHY_Prop_3D_np_4, PHY_Prop_3D_np_5, PHY_Prop_3D_np_6, PHY_Prop_3D_np_7),axis=2)
    if (args.PSSM == True):return np.concatenate((PSSM_3D_np_1, PSSM_3D_np_2,
            ...
            PSSM_3D_np_17, PSSM_3D_np_18, PSSM_3D_np_19, PSSM_3D_np_20), axis=2)


def enter(ECO=False, HSP=False, RAA=False, Pro2Vec_1D=False, PSSM=False,WINDOW_SIZE=1,
          RSA=False, Anchor=False, HYD=False, PKA=False, PHY_Char=False, PHY_Prop=False,POSITION=False):
    logging.basicConfig(format='[%(levelname)s] line %(lineno)d: %(message)s', level='INFO')
    log_time("Program started")
    parser = GetProgramArguments(ECO,RAA,HSP,Pro2Vec_1D,PSSM,WINDOW_SIZE,RSA,Anchor,HYD,PKA,PHY_Char,PHY_Prop,POSITION)
    ...
    print('****************')
    print('After computing your selected features, it has the shape:',test_all_features_np3D.shape)
    print('****************')
    # np.save(args.out_dir + '/biological_features.npy', np.array(test_all_features_np3D))

    log_time("Program ended")
    return test_all_features_np3D
